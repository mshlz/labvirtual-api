...
export class PageService extends BaseResourceService {
  ...
  public async list(page?: number, per_page?: number): Promise<any> {
    return super.list(page, per_page, {
      populate: [
        { path: "disciplines", select: "name id" },
        { path: "subjects", select: "name id" },
      ],
    });
  }
  ...
  public async get(id: string) {
    const result = await Page.findOne({ _id: id })
      .populate("disciplines", "name icon")
      .lean(true)
      .exec();
  ...
  public async getByCode(code: string) {
    const result = await Page.findOne({ code })
  ...
  public async getFromSubjects(subjects: string[]) {
    return Page.find({ subjects: { $in: subjects as any } })
